...
class LiabilityPortfolio(Portfolio):
    ...
    So there is only one active option added into liability portfolio at the beginning of a new episode.
...
class MainPortfolio(AssetInterface):
    ...
    def __init__(self, utils):
        ...
        # if performing an underlying price stress test, the structural change of price will be injected here
        self.crash_day = None
        if getattr(utils, "stress_test", False):
            T = self.a_price.shape[1]
            self.crash_day = np.random.randint(10, T - 5)
            self.a_price[:, self.crash_day + 1:] *= (1.0 - 0.1)
    ...
    def get_state(self, t):
        ...
        #turn all variables into array
        price = np.atleast_1d(price)
        gamma = np.atleast_1d(gamma)
        hed_gamma = np.atleast_1d(hed_gamma)
        states = np.concatenate([price, gamma, hed_gamma])
        if FLAGS.vega_obs:
            vega = self.get_vega(t)
            hed_vega = self.hed_port.options[self.sim_episode, t].vega_path[t] * self.utils.contract_size
            vega = np.atleast_1d(vega)
            hed_vega = np.atleast_1d(hed_vega)
    ...
    def step(self, action, t, result):
        ...
        result.stock_position = self.underlying.position = -1 * (
                    self.hed_port.get_delta(t) + self.liab_port.get_delta(t))
        # result.stock_position = self.underlying.position = 0
