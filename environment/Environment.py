...
class TradingEnv(gym.Env):
    ...
    def __init__(self, utils, logger = None):
        ...
        obs_lowbound = np.array([self.portfolio.a_price.min(),
                                 ...
                                 -np.inf])
        # array.min() return the globle min in array.
    ...
    def step(self, action):
        ...
        # action constraints
        gamma_action_bound = float(-self.portfolio.get_gamma(self.t) / \
                             self.portfolio.hed_port.options[self.sim_episode, self.t].gamma_path[
                                 self.t] / self.utils.contract_size)
        action_low = [0, gamma_action_bound]
        action_high = [0, gamma_action_bound]

        if FLAGS.vega_obs:
            # vega bounds
            vega_action_bound = float(-self.portfolio.get_vega(self.t) / \
                                self.portfolio.hed_port.options[self.sim_episode, self.t].vega_path[
                                    self.t] / self.utils.contract_size)
