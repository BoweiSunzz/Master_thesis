...
class Utils:
    def __init__(self, init_ttm, np_seed, num_sim, mu=0.0, init_vol=0.3,
                 ...
                 gbm=False, sabr=True, stress_test = False):
        if ttms is None:
            # ttms = [60, 120, 240, 480]
            ttms = [120]  # single ttm
        # Annual Return
    ...
    def _brownian_sim(self):
        """
        generate underlying price following GBM
        :return: asset price: nd.array(simulation path , period+1)
    ...
    def bs_call(iv, ttm: np.ndarray, S: np.ndarray, K, r, q, T=250):
        ...
        Args:
            ...
            ttm (np.ndarray): time to maturity in days, it should have the same channel dim as stock price S.
        ...
        # active option
        active_option = (ttm > 0).astype(np.uint8)
        matured_option = (ttm == 0).astype(np.uint8)
    ...
    def _sabr_sim(self):
        ...
        Returns:
            np.ndarray: stock price in shape (num_path, num_period+1)
            np.ndarray: instantaneous vol in shape (num_path, num_period+1)
        """
        qs = np.random.normal(size=(self.num_sim, self.num_period + 1))
        qi = np.random.normal(size=(self.num_sim, self.num_period + 1))
        qv = self.rho * qs + np.sqrt(1 - self.rho * self.rho) * qi # generating two Wiener processes with constant correlation rho
        ...
        for t in range(self.num_period):
            ...
            a_price[:, t + 1] = a_price[:, t] * np.exp(
                (self.mu - (gvol ** 2) / 2) * self.dt + gvol * np.sqrt(self.dt) * qs[:, t]    #use log Euler-Maruyama discretization to simulate price path
            )
    ...
    def get_sim_path_sabr(self):
        ...
        Returns:
            np.ndarray: underlying asset price in shape (num_path, num_period+1)
            np.ndarray: implied volatility in shape (num_path, num_period+1)
        """
    ...
    def atm_hedges(self, a_prices, vol):
        ...
        inactive_option = np.empty_like(price, dtype=np.bool)
    ...
    def agg_poisson_dist(self, a_prices, vol):
            ...
            if step_i > 0:
                if step_num_opts > 0:
                    step_next_port_price = step_port_price[:, :(-step_num_opts)].sum(
                        axis=1)  # only consider the positions from last step
                else:
        ...
        for ep_i in tqdm(range(num_episode)):
            options.append(
                SyntheticOption(port_price[ep_i, :], next_port_price[ep_i, :], port_delta[ep_i, :], port_gamma[ep_i, :],
                                port_vega[ep_i, :],
                                np.zeros((num_step,)).astype(np.bool), self.num_conts_to_add, self.contract_size))
        return np.array(options)